...
//!                              │
//!                              ▼
//!     data/derived/color_names.ron    ← generated, published to consumers
//! ```
//!
...
pub fn run(api_key: &str, dry_run: bool) -> Result<()> {
    ...
    let ron_path = ws.join("data/derived/color_names.ron");
...
//
// GENERATED in bkfunk/brickdata by `just refresh-color-names` from
//   data/rebrickable/colors.json      (cached Rebrickable API listing)
// ∖ data/rebrickable/color_excludes.ron
//
// This file is a PUBLISHED ARTIFACT: Blockstar vendors it into
// blockstar-core (see blockstar#143). Do not move it without updating
// that consumer -- the vendor recipe fetches this exact path.
//
// Do not edit by hand. To exclude a color, add its `rb_color_id` to
// `data/rebrickable/color_excludes.ron` and re-run `just refresh-color-names`.
//
